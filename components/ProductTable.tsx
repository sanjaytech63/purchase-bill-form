...
import { FormErrors, getProductError, getProductsArrayError } from '@/lib/errorUtils';
...
const ProductTable: React.FC<ProductTableProps> = ({
    ...
    const calculateAmount = (qty: number, freeQty: number, rate: number, discount: number): number => {
                ...
                return (
                    <div>
                        <Input
                            type="number"
                            value={value || ''}
                            onChange={(e) => updateProduct(record.id, 'qty', Number(e.target.value))}
                ...
                return (
                    <div>
                        <Input
                            type="number"
                            value={value || ''}
                            onChange={(e) => updateProduct(record.id, 'freeQty', Number(e.target.value))}
                            onKeyPress={(e) => handleKeyPress(e, record.id)}
                            min={0}
                            status={error ? 'error' : ''}
                        />
                ...
                return (
                    <div>
                        <Input
                            type="number"
                            value={value || ''}
                            onChange={(e) => updateProduct(record.id, 'rate', Number(e.target.value))}
                            onKeyPress={(e) => handleKeyPress(e, record.id)}
                            min={0}
                            step="0.01"
                            status={error ? 'error' : ''}
                ...
                return (
                    <div>
                        <Input
                            type="number"
                            value={value || ''}
                            onChange={(e) => updateProduct(record.id, 'discount', Number(e.target.value))}
                            onKeyPress={(e) => handleKeyPress(e, record.id)}
                            min={0}
                            max={100}
                            step="0.01"
    ...
    return (
        <div className="mb-6">
            ...
            <div className="py-4 text-right">
                <strong>
                    Total Amount: ₹{products.reduce((sum, product) => sum + product.amount, 0).toFixed(2)}
                </strong>
            </div>
        </div>
    );
